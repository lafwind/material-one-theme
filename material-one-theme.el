;;; material-one-theme.el --- A UI Theme Inspired by Material

;; Copyright (C) 2015 Lafwind
...
(deftheme material-one
  "A UI Theme Inspired by Material for Emacs")
...
(let* ((class '((class color) (min-colors 89)))
  ...
  (custom-theme-set-faces
   ...
   ;; Search
   `(match ((,class (:foreground ,background :background ,m_pink :inverse-video nil))))
   `(isearch ((,class (:foreground ,background :background ,m_pink))))
   `(isearch-lazy-highlight-face ((,class (:foreground ,foreground :background ,m_pink :inverse-video nil))))
